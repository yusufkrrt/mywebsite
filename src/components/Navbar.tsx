            ...
            <div className="hidden md:flex flex-1 items-center justify-end space-x-4">
              ...
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded-full p-1 border border-slate-200 dark:border-slate-800">
                <button
                  ...
                  className={`p-1.5 rounded-full transition-colors ${theme === 'light' ? 'bg-white shadow text-amber-500' : 'text-slate-500'}`}
                >
                  <Sun size={16} />
                </button>
                <button
                  onClick={() => setTheme('system')}
                  className={`p-1.5 rounded-full transition-colors ${theme === 'system' ? 'bg-white dark:bg-slate-800 shadow text-blue-500' : 'text-slate-500'}`}
                >
                  <Monitor size={16} />
                </button>
                <button
                  onClick={() => setTheme('dark')}
                  className={`p-1.5 rounded-full transition-colors ${theme === 'dark' ? 'bg-slate-800 shadow text-indigo-400' : 'text-slate-500'}`}
                >
                  <Moon size={16} />
                </button>
              </div>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navLinks.map((link) => (
                <a
                  ...
                  className="text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900 block px-3 py-2 rounded-md text-base font-medium"
                >
                  {link.name}
                </a>
              ))}
